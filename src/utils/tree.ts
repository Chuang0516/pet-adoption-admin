 ...
 * @Description  : 将树形结构处理为：label-value的形式
 */
export const treeToSelect = (treeDatas: any[], config: any = {
  value: 'value',
  label: 'label',
  children: 'children',
  key: 'id'
  ...
  // 结果数组
  const { value = 'value', label = 'label', children = 'children', key = 'id' } = config
  if(value == 'item') {
    return treeDatas.map(item => {
      if(item.children && item.children.length) {
        return {
          value: item,
          label: item[label],
          children: treeToSelect(item[children], config),
          key: item[key]
        }
      }
      return {
