        ...
        default: () => {
          return (
            <>
              <div class="flex">
                <div>
                  <ElInput v-model={code.value} placeholder='请输入计算结果'></ElInput>
                </div>
                <div v-html={captchaSvg.value} style="height: var(--el-component-size-large);" class="cursor-pointer svg-w-[100%] svg-h-full" onClick={getCaptcha}></div>
              </div>
            </>
          )
        }
...
const getCaptcha = () => {
  getCaptchaApi().then(res => {
    if (res.code == 200) {
      captchaSvg.value = res.data.data
...
const getRole = async () => {
  ...
  if (res) {
    ...
    permissionStore.setIsAddRouters(true)
    console.log({ path: redirect.value || permissionStore.addRouters[0].path }, '{ path: redirect.value || permissionStore.addRouters[0].path }');
