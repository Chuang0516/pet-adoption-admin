              ...
              <div class="flex w-full">
                <div class="flex-1">
                  ...
                  >
                    {{
                      default: (data) => {
                        return <span>{data.meta.title}</span>
                      }
                    }}
                  </ElTree>
                </div>
...
const getMenuList = async () => {
  ...
  if (res) {
    ...
    eachTree(props.currentRow.menus, (v) => {
      console.log(props.currentRow.menus, 'props.currentRow.menus');
...
const submit = async () => {
  ...
  if (valid) {
    ...
    const checkedNodes = unref(treeRef)?.getCheckedNodes() || []
    // const data = filter(unref(treeData), (item: any) => {
    //   return checkedKeys.includes(item.id)
    // })
    formData.menus = checkedNodes || []
    return formData
  }
}
