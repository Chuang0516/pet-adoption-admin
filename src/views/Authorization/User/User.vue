...
import { ElTree, ElInput, ElDivider, ElTag, ElSelect, ElOption } from 'element-plus'
import { getDepartmentApi, getUserPageApi, saveUserApi, deleteUserByIdApi } from '@/api/department'
    ...
    search: {
      componentProps: {
        slots: {
          append: () => (
            <ElSelect
              v-model={searchSql.value.username}
              placeholder="Select"
              style="width: 115px"
            >
              <ElOption label="等于" value="=" />
              <ElOption label="不等于" value="!=" />
              <ElOption label="模糊查询" value="like" />
            </ElSelect>
          )
        }
      }
